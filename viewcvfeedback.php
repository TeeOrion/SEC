...
<div class="Online"><p2> CV Feedback </p2></div>
</form>
...
$kuid = $_SESSION['kuid'];
  $statement = $conn->prepare("SELECT CVFeedback.fbID, CVFeedback.cvID, CVFeedback.userID, CVFeedback.feedback FROM CVFeedback, CV WHERE CVFeedback.userID = '$kuid' AND CVFeedback.cvID = CV.cvID");
$statement->execute();
$CVFeedback = array();
while (($row = $statement->fetch(PDO::FETCH_ASSOC)) !== false) {
    $CVFeedback           = new CVFeedback();
    $CVFeedback->cvID    = $row['CVFeedback.cvID'];
    $CVFeedback->userID    = $row['CVFeedback.userID'];
    $CVFeedback->jobDesc  = $row['CVFeedback.feedback'];
